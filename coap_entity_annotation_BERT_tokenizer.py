...
from transformers import BertTokenizer, TFAutoModel, AutoConfig
import string
import nltk
import os
...
sentence_text = sentence_text[:1270]
...
embeddings = bert.bert(input_ids=inputs, attention_mask=mask)[0]
...
#
best_model_callback = tf.keras.callbacks.ModelCheckpoint("./model/entity_extraction", save_best_only=True,
                                                         monitor="val_loss", mode="min")
model.fit([x_train, x_train_mask], y_train, validation_data=([x_val, x_val_mask], y_val),
          callbacks=[best_model_callback], batch_size=8, epochs=10)
# # #
