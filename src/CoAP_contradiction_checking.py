...
coap_entity_indexes = pickle.load(open("../data/coap_sentence_entity_indexes", "rb"))
coap_contexts_condition_split = pickle.load(open('../data/coap_contexts_condition_split', 'rb'))
coap_contexts_split_properties = pickle.load(open('../data/coap_split_properties', 'rb'))
...
for i in range(len(rfc7252)):
    sentence_id = input_ids[i]
    entity_indexes = coap_entity_indexes[i]
    for entity_index in entity_indexes:
        start = entity_index[0]
        end = entity_index[1] + 1
        entity = tokenizer.decode(sentence_id[start:end])
        if entity != "":
...
# Create a graph with all the entities
graph = nx.Graph()
...
for i in range(len(coap_contexts_split_properties)):
    atomic_rules_list = coap_contexts_split_properties[i].split(";")[:-1]
    ...
    for atomic_rule in atomic_rules_list:
        ...
        if type(var_op_val) == tuple:
            ...
            transformed_atomic_rules_list.append(
                AtomicRule(var, op, value, coap_contexts_condition_split[i][-1], entity))
        else:
...
for i in range(len(all_transformed_rules_list)):
    entity_atomic_rules = {}
    condition = coap_contexts_condition_split[i][2]
    rule_sentence = rfc7252_rule_sentences[coap_contexts_condition_split[i][-1]]
...
# #         var_op_val = []
# #         for atomic_rule in atomic_rules:
# #             if atomic_rule.entity == "coap version number":
# #                 var_op_val.append((atomic_rule.var, atomic_rule.op, atomic_rule.value))
# #         var_op_val_list.extend(var_op_val)
# # print(var_op_val_list)
#
...
#
# Now we check contradictions. We use a top-down approach. We first check which entities have potential contradictions,
# then narrow down to the rules and identify the detailed rules and rule sentences
#
# Use z3 solver to create literals, clauses and check satisfiability. The literals are all float literals
#
