...
entities = pickle.load(open("../data/all_MQTT_entities", "rb"))
MQTT_context_condition_split = pickle.load(open('../data/mqtt_contexts_condition_split', 'rb'))
MQTT_split_properties = pickle.load(open('../data/all_MQTT_properties', 'rb'))
...
for i in range(len(MQTT_split_properties)):
    atomic_rules_list = MQTT_split_properties[i].split(";")[:-1]
    ...
    for atomic_rule in atomic_rules_list:
        ...
        if type(var_op_val) == tuple:
            ...
            transformed_atomic_rules_list.append(
                AtomicRule(var, op, value, MQTT_context_condition_split[i][-1], entity))
        else:
...
for i in range(len(all_transformed_rules_list)):
    entity_atomic_rules = {}
    condition = MQTT_context_condition_split[i][2]
    rule_sentence = mqtt_rule_sentences[MQTT_context_condition_split[i][-1]]
